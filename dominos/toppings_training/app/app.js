...
function _SOLUTION_PIZZA_FEAST_(pizzaName, q) {
   ...
   let skip_base_query = !q.some(item => Object.keys(SAUCES).includes(item));
    ...
    _WRAPPER_.className = "spf-wrapper"
        ...
        for (let i = 0; i < TOPPINGS_LIST_MEATS.length; i++) {
            // console.log(TOPPINGS_LIST_MEATS[i]);
            
            _SPF_(q, TOPPINGS_LIST_MEATS[i], q_filtered, _EXTRA_, pizzaName, skip_base_query, _SPF_TOPPINGS_WRAPPER_, "meat-symbol", i+1)
        }
        for (let i = 0; i < TOPPINGS_LIST_VEGETABLES.length; i++) {
            _SPF_(q, TOPPINGS_LIST_VEGETABLES[i], q_filtered, _EXTRA_, pizzaName, skip_base_query, _SPF_TOPPINGS_WRAPPER_, "veggie-symbol", i+1)
        }
        for (let i = 0; i < TOPPINGS_LIST_CHEESES.length; i++) {
            _SPF_(q, TOPPINGS_LIST_CHEESES[i], q_filtered, _EXTRA_, pizzaName, skip_base_query, _SPF_TOPPINGS_WRAPPER_, "cheese-symbol", i+1)
        }
        // TOPPINGS_LIST_MEATS.forEach(topping => {
    ...
    function meat_listener(e) {
        if (e.keyCode >= 49 && e.keyCode <= 57) document.querySelectorAll(".meat-symbol")[e.key-1].click()
    }
    addEventListener("keydown", meat_listener)

    function veggie_listener(e) {
        if (e.keyCode >= 49 && e.keyCode <= 57) document.querySelectorAll(".veggie-symbol")[e.key-1].click()
    }
    addEventListener("keydown", veggie_listener)

    function cheese_listener(e) {
        if (e.keyCode >= 49 && e.keyCode <= 57) document.querySelectorAll(".cheese-symbol")[e.key-1].click()
    }
    addEventListener("keydown", cheese_listener)
    ...
    _TABS_.addEventListener("click", () => {
        ...
        switch (TAB) {
            case 0:
                addEventListener("keydown", meat_listener)
                removeEventListener("keydown", veggie_listener)
                removeEventListener("keydown", cheese_listener)
            ...
            case 1:
                addEventListener("keydown", veggie_listener)
                removeEventListener("keydown", meat_listener)
                removeEventListener("keydown", cheese_listener)
...
let _SPF_FOUND_ = 0
function _SPF_(q, topping, q_filtered, _EXTRA_, pizzaName, skip_base_query, _SPF_TOPPINGS_WRAPPER_, classname, i) {
